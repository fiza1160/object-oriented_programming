...
# Levels_ex['levels'].append({'map': _map, 'obj': _obj})
#
...
def medium_level(loader, node):
    ...
    obj.config = pars_node(loader, node)
    return {'map': map, 'obj': obj}
...
def pars_node(loader, node):
    data = loader.construct_mapping(node)
    data['enemy'] = loader.construct_sequence(node.value[0][1])
    return data


loader = yaml.Loader
loader.add_constructor("!easy_level", easy_level)
loader.add_constructor("!medium_level", medium_level)
loader.add_constructor("!hard_level", hard_level)
Levels = yaml.load(level_yaml)
